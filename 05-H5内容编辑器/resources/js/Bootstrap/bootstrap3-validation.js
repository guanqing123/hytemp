...
!function($) {
    ...
    $.fn.valid = function (options) {
        ...
        $('input, textarea', this).each(function () {
            ...
            //debugger; why has success not valid again?    !controlGroup.hasClass('has-success') && 
   ...
   $.fn.validation.defaults = {
        validRules : [
            { name: 'required', validate: function (value) { return ($.trim(value) == ''); }, defaultMsg: '该项必填' },
            { name: 'compared', validate: function (value) { return (value != $($(this).attr("data-match")).val()); }, defaultMsg: '两次输入不一致' },
            ...
            {name: 'mail', validate: function(value) {return (!/^((([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+(\.([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\x0e-\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))$/i.test(value));}, defaultMsg: '邮箱格式错误'},
            { name: 'char', validate: function (value) { return (!/^[a-z\_\-A-Z]*$/.test(value)); }, defaultMsg: '请输入英文字符' },
            { name: 'mobile', validate: function (value) { return (!/^1[3|4|5|6|7|8|9][0-9]{1}[0-9]{8}$/.test(value)); }, defaultMsg: '手机号码格式错误' },
            { name: 'postcode', validate: function (value) { return (!/^[1-9]\d{5}$/.test(value)); }, defaultMsg: '邮政编码格式错误' },
            { name: 'idcard', validate: function (value) { return (!/^(\d{15}$|^\d{18}$|^\d{17}(\d|X|x))$/.test(value)); }, defaultMsg: '身份证格式错误' },
            { name: 'money', validate: function (value) { return (!/^(([1-9]\d*)|\d)(\.\d{1,2})?$/.test(value)); }, defaultMsg: '金额格式错误' },
            {name: 'chinese', validate: function(value) {return (!/^[\u4e00-\u9fff]$/.test(value));}, defaultMsg: '请输入汉字'},
            {name: 'url',validate:function(value){return(!/^(https?|s?ftp):\/\/(((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:)*@)?(((\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5]))|((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?)(:\d*)?)(\/((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)+(\/(([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)*)*)?)?(\?((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|[\uE000-\uF8FF]|\/|\?)*)?(#((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|\/|\?)*)?$/i.test(value))},defaultMsg:'网址格式错误'},
            { name: 'date', validate: function (value) { if (value == "") return false;else return(/Invalid|NaN/.test(new Date(value).toString()));},defaultMsg:"日期格式XXXX-XX-XX"}
        ],
        reqmark:true,
    ...
    var validateField = function(field, valid) { 
            ...
            for (j = 0; j < rules.length; j++) {
                ...
                if (flag == rule.name) {
                    ...
                    if (rule.validate.call(field, value) == x) {
                        error = true;
                        if (el.attr('type')!=undefined && el.attr('type').toLowerCase()=='file'){
        ...
        var controlGroup = el.parent();//el.parents('.form-group');
        if (el.parent().hasClass("input-group")) {
            el.parent().parent().find("#valierr").remove();
        }
        controlGroup.removeClass('has-error has-success');
        controlGroup.addClass(error==false?'':'has-error');
...
(function ($) {
    ...
    $.fn.setRecord = function (record) {
        //取出验证的
        $('input,select,textarea,radio', this).each(function () {
            var el = $(this);
            if (el.attr("name") != "__RequestVerificationToken") {//排除token
                var value = record[el.attr("name")] == null ? "" : record[el.attr("name")];
    ...
    $.fn.resetForm = function () {
        $(this)[0].reset();
        $(this).find("input[type=hidden][name!='__RequestVerificationToken']").val("");
        $(this).find("input[type=checkbox]").removeAttr("checked");
        $("[id='valierr']").remove();
        $.each($(this).find(".form-group"), function (idx, item) {
