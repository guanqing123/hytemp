require(['httpKit'], function (httpKit) {
     new Vue({
        ...
        created(){
        	window.localStorage.setItem('paystatus', 0);
        	var self = this;
            ...
            self.$nextTick(function () {
            	var payResult = function () {
            		self.$toast.success('hhh');
	         		...
	         		httpKit.post("/coffee/pay/result", data, httpKit.type.form).then(res => {
	         			self.$toast.clear();
	         			self.status = parseInt(res.data.paystatus);
	         			if (self.status == 1) {
	         				setTimeout(payResult, 5000);
	         			} else {
	                    ...
	                    self.$dialog.alert({
	                    	message: '获取支付状态异常,请查看订单列表',
	                    }).then(() => {
	                        window.location.href='../gcoffeeOrder/gcoffeeOrder.html'
	                    });
	         		});
         		}
         		if (!window.localStorage.getItem('paystatus')) {
         			payResult();
         		}
                /*var count = 0;
                var timeStamp = null;
...
/*                (function (self) {
                    ...
                    var count = 0;
                    self.execTimer = function(fun, timer) {
                        var self = this;
                        self.timeStamp = setTimeout(function(){
                            count++;
                            if (count <= (3 * 60 * 1000 / timer)) {
                                fun(self);
                                self.execTimer(fun(self), timer);
                            } else {
                                clearTimeout(self.timeStamp);
                            }
                        }, timer);
                    ...
                    self.execTimer((tar)=>{
                        ...
                        httpKit.post("/coffee/pay/result", data, httpKit.type.form).then(res => {
                            self.$toast.clear();
                            self.status = res.data.paystatus * 1 + 1;
                            if (self.status == 2) {
                                clearTimeout(self.timeStamp);
                            }
                            self.$toast.success('s='+self.status)
                        }).catch(err => {
                            self.$toast.clear();
                            self.$toast.fail({
                                message: err.message
